<template>
    <el-main class="main-box">
      <div class="operation-box">
        <el-button type="primary" @click="addOrEdit('/video-manage-detail')">新建</el-button>
        <el-button @click="batchDelete(selectedList, true, message)">批量删除</el-button>
      </div>
      <div class="search-box">
        <el-input class="search-input" v-model="searchForm.name" placeholder="请输入视频名称"></el-input>
      ...
      <el-table
        ...
        <el-table-column prop="name" sortable="custom" label="视频名称"></el-table-column>
        <el-table-column prop="videoCode" sortable="custom" label="视频编码"></el-table-column>
        <el-table-column prop="imageUrl" sortable="custom" width="125px" label="课程视频">
          <template v-slot="{row}">
            <img :src="row.imageUrl" :alt="row.name" style="width: 100px;height: auto" @click="showVideo(row.imageUrl,row.videoUrl)">
          </template>
        </el-table-column>
        <el-table-column prop="belongCourseName" sortable="custom" label="视频编码"></el-table-column>
        <el-table-column prop="belongTeacherName" sortable="custom" label="视频编码"></el-table-column>
        <el-table-column prop="operation" label="操作">
          <template v-slot="{row}">
            <el-button type="text" class="jh-button-wider" @click="addOrEdit('/video-manage-detail', row.id)">修改</el-button>
